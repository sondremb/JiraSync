...
The status codes are defined by [RFC 9110]({@link https://httpwg.org/specs/rfc9110.html#overview.of.status.codes}).
...
export enum StatusCode {
	...
	/** This response code is used for caching purposes. It tells the client that the response has not been modified, so the client can continue to use the same cached version of the response.
	 *
	...
	/** This response is sent when the client must first authenticate itself with the proxy.
	 *
	 * {@link https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/407}
	 */
	ProxyAuthenticationRequired407 = 407,

	/** This response is sent when the server would like to shut down an idle connection.
	 *
	...
	/** The client request declares an HTTP Extension ([RFC 2774]({@link https://datatracker.ietf.org/doc/html/rfc2774})) that should be used to process the request, but the extension is not supported.
	 *
